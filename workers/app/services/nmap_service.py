...
SCAN_PROFILES = {
    "standard": ("-Pn -T4 --version-light -sV --top-ports 400 "),
...
def run_live_nmap_scan(
    ...
    (logger.info(f"[NMAP_Service] Starting '{profile}' scan for IP address: {ip_address}"))
    ...
    try:
        ...
        (result["ports"].sort(key=lambda port: port["port"]))
        ...
        logger.exception(f"[NMAP_Service] PortScanner error while scanning {ip_address}: {error}")
        raise

    except Exception as error:
        logger.exception(f"[NMAP_Service] Unexpected error while scanning {ip_address}: {error}")
        raise
