...
Performance testing is conducted using k6 for measuring API
response times, latency percentiles, and throughput under load. Test
scripts define pass/fail thresholds directly (e.g. `p(95)<2000` for
API response time), allowing performance tests to be run as a gating
step in the CI pipeline. WebSocket latency for real-time scan progress
...
against the staging API, using virtual users (VUs) with staged ramp-up
scheduling to simulate concurrent load. Test scripts simulate realistic
traffic patterns including scan initiation, report retrieval, and scan
history queries across multiple concurrent virtual users.
...
- **Generic Vulnerability Scanning:** Automated baseline scanning for
  missing security headers (CSP, HSTS, X-Frame-Options), insecure
  cookie flags, and common injection vectors (XSS, SQLi) across
...
In addition, OWASP ZAP baseline scans are run against the staging
deployment to detect generic web vulnerability classes that fall
outside the scope of business-logic-specific pytest assertions, such
as missing security headers, insecure cookie configuration, and common
injection points. Where authenticated coverage is required, ZAP is run
with a context file and valid session credentials; otherwise the
baseline scan runs unauthenticated against publicly reachable routes.
...
- OWASP ZAP baseline scan reports zero medium-or-above risk alerts on
  staging
---
