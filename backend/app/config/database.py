...
def get_database_url() -> str:
    ...
    values = {variable: os.getenv(variable) for variable in required_variables}

    missing_vars = [variable for variable, value in values.items() if not value]

    if missing_vars:
        raise RuntimeError("Database config is missing. Missing: " + ", ".join(missing_vars))
